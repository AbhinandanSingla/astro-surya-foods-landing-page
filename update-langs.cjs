...
let siteFile = path.join(__dirname, 'src/consts/site.ts');
let content = fs.readFileSync(siteFile, 'utf8');

const regex = /export const LANGUAGES = \[[\s\S]*?\] as const;/;

const newArrStr = `export const LANGUAGES = [\n${languages.map(l => `  { code: "${l.code}", label: "${l.label}" },`).join('\n')}\n] as const;`;

const newContent = content.replace(regex, newArrStr);

fs.writeFileSync(siteFile, newContent);
console.log('Done');
